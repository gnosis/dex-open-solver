...
def find_best_buy_amounts(xrate, b_orders, s_orders, fee_ratio=F(0)):
    ...
    # Remove orders that will violate the limit exchange rate after
    # the fee is deducted.
    b_orders = [
    ...
    while s_i < len(s_orders) and b_i < len(b_orders):
        b_buy_amount_ub = order_sell_amount(b_orders[b_i]) / xrate - b_buy_amounts[b_i]
        s_buy_amount_ub = order_sell_amount(s_orders[s_i]) * xrate - s_buy_amounts[s_i]
        assert b_buy_amount_ub >= 0 and s_buy_amount_ub >= 0
        # Check which order gets completely filled first
        if b_buy_amount_ub < s_buy_amount_ub / xrate:
            b_buy_amounts[b_i] += b_buy_amount_ub
            s_buy_amounts[s_i] += b_buy_amount_ub * xrate
            b_i += 1
        elif b_buy_amount_ub > s_buy_amount_ub / xrate:
            b_buy_amounts[b_i] += s_buy_amount_ub / xrate
